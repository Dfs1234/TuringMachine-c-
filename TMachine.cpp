...
std::vector<std::string> TuringMachine::getAllTransition()const{ return allTransition; }
...
bool TuringMachine::isAccept(Tape tape, TMTable TMtable){
    while(currentState != '7'){
        std::string transition = getTransition(tape.getSymbol());
        tape.printTape();
        TMtable.print(allTransition,currentState, tape.getSymbol());
